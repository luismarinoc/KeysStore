...
import { Credential } from '@keysstore/shared-types';
...
interface CredentialContextType {
    ...
    addCredential: (data: Omit<Credential, 'id' | 'created_at' | 'user_id'>) => Promise<void>;
...
export const CredentialProvider = ({ children }: { children: ReactNode }) => {
    ...
    const addCredential = async (data: Omit<Credential, 'id' | 'created_at' | 'user_id'>) => {
        if (!user) {
